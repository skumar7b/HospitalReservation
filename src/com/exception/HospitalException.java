...
public class HospitalDAO {
	 ...
	 * HotelException { boolean result=false; ConnectionManager connectionGetter =
	 * null; Connection connection = null; ResultSet resultSet = null;
	 * PreparedStatement preparedStatement = null;
	 * 
	 * String newPassword=null; try { try { connectionGetter = new
	 * ConnectionManager(); connection = connectionGetter.getConnection();
	 * //System.out.println(connection); preparedStatement = connection.
	 * prepareStatement("select password from login_hotel_802366 where USERNAME=?");
	 * preparedStatement.setString(1,userName);
	 * 
	 * resultSet=preparedStatement.executeQuery(); if(resultSet.next()) {
	 * newPassword=resultSet.getString(1); if(passWord.equals(newPassword)) {
	 * result=true;
	 * 
	 * } else { result=false;
	 * 
	 ...
	 * e.printStackTrace(); } finally { if(resultSet != null) resultSet = null;
	 * if(preparedStatement != null) preparedStatement.close(); if(connection !=
	 * null) connection.close(); } } catch(SQLException exception) { throw new
	 * HotelException("Sql connection problem");
	 * 
	 * } return result; }
	 */
